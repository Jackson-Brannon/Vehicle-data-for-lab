...
// Steps are not in order, but code warranted mixing orders to avoid declaration issues

// Add recall reason from the recallList to each vehicle (based on VIN) as "recallReason"
...
const filteredVehicles = vehicles.filter(vehicle => !salvageVINs.includes(vehicle.vin));

// Create new arrays for each "Make" of vehicle.
const filteredMakesAndModels = {};
for (const vehicle of filteredVehicles) {
  if (!filteredMakesAndModels[vehicle.make]) {
    filteredMakesAndModels[vehicle.make] = [];
  }
  ...
  if (vehicle.recallReason) {
    model.recallReason = vehicle.recallReason;
  }
  
  filteredMakesAndModels[vehicle.make].push(model);
}

// Order each new Make array by vehicle year in ascending order
for (const make in filteredMakesAndModels) {
  filteredMakesAndModels[make].sort((a, b) => (a.year > b.year ? 1 : -1));
}
...
//    d.  Total number of non-salvage vehicles of each year model, regardless of Make.
for (const make in filteredMakesAndModels) {
  const vehicles = filteredMakesAndModels[make];
  console.log(`${make} (model, year, vin, recallReason):\n`);
  for (const vehicle of vehicles) {
    const { model, year, vin, recallReason } = vehicle;
    console.log(`\t${model}, ${year}, ${vin}, ${recallReason || "N/A"}`);
      console.log(`\n`); 
}
}

// There's a lot of declarations and console.logs here, but this is the way I found it to work. I may revisit to look into formatting further if possible
const totalVehicles = vehicles.length;
const totalNonSalvageVehicles = filteredVehicles.length;
const totalSalvageVehicles = totalVehicles - totalNonSalvageVehicles;

const makesCounts = {};
const yearsCounts = {};

for (const vehicle of filteredVehicles) {
  if (!makesCounts[vehicle.make]) {
    makesCounts[vehicle.make] = 0;
  }
  makesCounts[vehicle.make]++;
  
  if (!yearsCounts[vehicle.year]) {
    yearsCounts[vehicle.year] = 0;
  }
  yearsCounts[vehicle.year]++;
}

console.log(`Total number of vehicles processed: ${totalVehicles}`);
console.log(`Total (non-salvage) number of each make:\n`);
for (const make in makesCounts) {
  console.log(`\t${make}: ${makesCounts[make]}`);
}
console.log(`\n`);
console.log(`Total number of vehicles removed due to salvage: ${totalSalvageVehicles}`);
console.log(`Total (non-salvage) number of each year:\n`);
for (const year in yearsCounts) {
  console.log(`\t${year}: ${yearsCounts[year]}`);
}
